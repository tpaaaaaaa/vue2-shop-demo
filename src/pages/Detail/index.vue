                ...
                <div class="InfoWrap">
                    ...
                    <div class="choose">
                        <div class="chooseArea">
                            ...
                            <dl v-for="attr of spuSaleAttrList" :key="attr.id">

                                <dt class="title">{{attr.saleAttrName}}</dt>

                                <dd changepirce="0"
                                    v-for="value of attr.spuSaleAttrValueList"
                                    :key="value.id"
                                    :class="{active:+value.isChecked}"
                        ...
                        <div class="cartWrap">
                            <div class="controls">
                                <input @change="changeSkuNum" autocomplete="off" :value="skuNum" class="itxt">
                                <a @click="skuNum++" class="plus">+</a>
                                <a @click="skuNum>1?skuNum--:1" class="mins">-</a>
                            </div>
                            <div class="add">
...
export default {
    ...
    computed: {
        ...mapGetters('detail', ['categoryView', 'skuInfo', 'spuSaleAttrList', 'valuesSkuJson']),
        skuImageList() {
    ...
    methods: {
        ...
        changeSkuNum(e) {
            ...
            this.skuNum = Number.parseInt(e.target.value);
            if (e.target.value < 1) this.skuNum = 1;
        },
        // 添加购物车
        async addShopCar() {
            try {
                await this.$store.dispatch('detail/addOrUpdateShopCart', { skuId: this.$route.params.skuId, skuNum: this.skuNum });
                // 路由跳转,将产品相关信息带给下一级的路由
                // 简单的数据skuNum通过query传给路由组件
                // 复杂的存到会话存储(不持久化，会话结束后消失)
                // 本地存储||绘画存储，一般是存储字符串
