<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address of addressInfo" :key="address.id">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="order of orderInfo.detailArrayList" :key="order.skuId">
          <li>
...
export default {
  ...
  computed: {
    ...
    // 返现
    saleAmount() {
      return Math.abs(this.orderInfo.totalAmount - this.orderInfo.originalTotalAmount) || '0.00';
    }
  ...
  methods: {
    ...
    changeDefault(address, addressInfo) {
      // 全部的isDefault为0
      addressInfo.map(item => item.isDefault = '0')
      address.isDefault = '1';
    },
    async submitOrder() {
      let tradeNo = this.orderInfo.tradeNo;
      let data = {
        consignee: this.userDefaultAddress[0].consignee,//收件人
        consigneeTel: this.userDefaultAddress[0].phoneNum,//手机号
        deliveryAddress: this.userDefaultAddress[0].fullAddress,//地址
      ...
      if (result.code !== 200) return console.log(result.message, result.data);
